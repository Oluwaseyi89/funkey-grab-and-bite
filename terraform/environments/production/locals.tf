locals {
  ...
  # ── Conditional module output helpers ──────────────────────────
  # Use try() so modules that are disabled (count=0) produce empty
  # values instead of errors in cross-module references.
  ...
  sqs_order_queue_arn    = try(module.sqs[0].order_queue_arn, "")
